...
;;; ------------------------------------------------------------------------
;;; We start by including oauth1.0a package developed by StephenCharles
;;; found: https://github.com/StephenCharles/Racket-OAuth1.0a to facilitate
;;; authentication and authorization to twitter REST API
;;; ------------------------------------------------------------------------
...
;;; 2.
;;; Read in the entire tweets collected based on the query
...
(define (tweet-text-only lst)
  ;;; This abstraction returns only the portion of the hash table that we need
  ;;; to inorder to pull out tweet text
  (define t (car (car
                  (let ([tmp (map (λ (x) (list (hash-ref x 'statuses))) lst)])
                    tmp))))
...
(define pure-tweet-text (tweet-list->string (tweet-text-only tweets)))
